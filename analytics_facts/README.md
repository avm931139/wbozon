...
- `fact_product_economics_daily` — ежедневная SKU-экономика: финансовая
  выручка, себестоимость, распределённые расходы, логистика, реклама и прибыль
  в копейках;
...
Общие расходы и логистика распределяются между SKU пропорционально положительной
чистой выручке внутри площадки и финансового дня. WB использует прямую товарную
рекламную статистику. Ozon загружает исторические Performance-отчёты с SKU
пакетами не более 62 дней и 10 кампаний; нетоварный остаток распределяется по
дневной финансовой выручке. У Яндекс Маркета буст продаж связывается напрямую
...
полю `cost`. Форматы без товарной детализации распределяются по выручке с явной
меткой метода.
