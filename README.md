...
- независимый планировщик текущих остатков и ежедневных срезов `inventory_sync`;
...
- ozon/
  ...
  - API-модули — каталог, остатки, отправления, поставки, обращения, аналитика и финансы.
...
- inventory_sync/
  - service.py — полная загрузка и атомарное сохранение остатков WB/Ozon.
...
Команда `python -m inventory_sync` должна работать постоянно как отдельный процесс. Успешные и неуспешные запуски сохраняются в `inventory_sync_runs`; при ошибке одного из трёх источников частичный ежедневный срез не записывается.
