...
- единый слой подтверждённых финансовых продаж, возвратов и множественных
  штрихкодов в пакете `analytics_facts`, с денежными суммами в целых копейках;
- нормализованные карточки и локальное хранилище фото/видео в пакете `product_catalog`;
...
- analytics_facts/
  - service.py — независимая по маркетплейсам нормализация финансовых продаж и возвратов;
  - `__main__.py` — построение `fact_sales`, lineage и справочника штрихкодов.
- healthcheck/
...
- `WB_TG_MESSAGE_DELAY_SECONDS` — задержка между отдельными сообщениями площадок и итогом, по умолчанию 3 секунды.
