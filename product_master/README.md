...
Подтверждённые межплощадочные псевдонимы задаются явно, а не определяются
нечётким сравнением: `8410/6+3 WHT` объединяется с `8410/6+3`, `MDL29829` —
с `NVL0056`, `MDL29929` — с `NVL0057`. Исходные артикулы и карточки
сохраняются в `marketplace_product_links`, поэтому объединение не уничтожает
историю площадок.
