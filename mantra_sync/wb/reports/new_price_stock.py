import pandas as pd
from sqlalchemy import and_, cast, Integer, func, desc, String
...
from core.db.models import (
    ...
    ParserProduct, MSStock, OzonCard
)
...
class WBStocksExcelExporter:
    ...
    def _extract_skus_from_sizes(self, sizes_data) -> str:
        """
        Извлекает SKU из поля sizes (JSON) в модели WbCard
        ...
        Args:
            sizes_data: JSON поле sizes из WbCard

        Returns:
            Строка со всеми SKU через запятую или пустая строка
        """
        if not sizes_data:
            return ""
        ...
        try:
            ...
            if isinstance(sizes_data, str):
                sizes_dict = json.loads(sizes_data)
            else:
                sizes_dict = sizes_data

            # Извлекаем skus из словаря
            if isinstance(sizes_dict, dict) and 'skus' in sizes_dict:
                skus = sizes_dict.get('skus', [])
                if isinstance(skus, list):
                    skus_list = [str(sku) for sku in skus if sku]
                    # Возвращаем уникальные SKU через запятую
                    unique_skus = list(dict.fromkeys(skus_list))  # Сохраняем порядок, убирая дубликаты
                    return ', '.join(unique_skus) if unique_skus else ""

            return ""

        except (json.JSONDecodeError, TypeError, AttributeError) as e:
            logger.warning(f"Ошибка при парсинге sizes: {e}, значение: {sizes_data}")
            return ""
    ...
    def _get_aggregated_data(self, session: Session) -> List:
        ...
        - Добавляем SKU из WbCard.skus и OzonCard
        """
        ...
        # Подзапрос для получения SKU из OzonCard
        # Связь через offer_id (артикул продавца) с vendor_code из WbCard
        ozon_skus = (
            session.query(
                OzonCard.offer_id,
                OzonCard.sku.label('ozon_sku')
            )
            .filter(OzonCard.sku.isnot(None))
            .subquery()
        )
        ...
        query = (
            session.query(
                ...
                WbCard.skus.label("wb_skus"),  # Добавляем SKU из WbCard
                ...
                ozon_skus.c.ozon_sku.label("ozon_sku")
            )
            .join(
                WBNormalizedProduct,
                and_(
            ...
            .join(
                ozon_skus,
                ozon_skus.c.offer_id == WbCard.vendor_code,
                isouter=True  # LEFT JOIN на случай отсутствия записей
            )
            .distinct()
        )
    ...
    def _prepare_dataframe(self, results) -> pd.DataFrame:
        ...
        for row in results:
            ...
            # Получаем SKU из WbCard (поле skus)
            wb_sku_value = row.wb_skus if row.wb_skus else ""

            # Получаем SKU из OzonCard
            ozon_sku_value = row.ozon_sku if row.ozon_sku else ""
            ...
            data.append({
                ...
                'SKU WB': wb_sku_value if wb_sku_value else '',
                'SKU Ozon': ozon_sku_value if ozon_sku_value else ''
            })
