...
Отчёт строится из уже сохранённых данных PostgreSQL и не выполняет запросы к
Ozon. Поэтому его можно формировать в любое время без риска HTTP 429:

```bash
cd /home/wbozon/wbozon
./.venv/bin/python -m ozon --supply-reconciliation-report
```

По умолчанию файл сохраняется в
`data/ozon/reports/ozon_fbo_supply_reconciliation_all.xlsx`. Другой путь можно
передать аргументом:

```bash
./.venv/bin/python -m ozon \
  --supply-reconciliation-report /home/wbozon/ozon_fbo_supplies.xlsx
```

Лист `По товарам` содержит отдельную строку для каждой комбинации поставки и
SKU: заявку и ID поставки, bundle, статус, склад, товар, отправленное,
фактически принятое и согласованное количество, расхождение, брак, излишек,
недостачу и реквизиты актов. Лист `По поставкам` суммирует те же показатели по
поставке. В отчёт попадает весь период, уже загруженный в таблицы сверки.
