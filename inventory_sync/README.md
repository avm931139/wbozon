...
При ошибке обязательного API срез не создаётся частично. Ошибка записывается в `inventory_sync_runs`, а планировщик повторяет дневной срез через `INVENTORY_SNAPSHOT_RETRY_SECONDS` (по умолчанию через 5 минут). Часовой интервал продолжает применяться к обычному обновлению текущих остатков.
