...
- активность `wbozon-wb.service`, независимых timers Order Feed и Sales Funnel,
  а также свежесть их последних запусков;
- активность Telegram worker, stock timer и, если настроен proxy, SSH relay;
- активность ежедневного timer документов WB и свежесть его журнала, если `WB_DOCUMENT_SYNC_REQUIRED=true`;
- свежесть нормализации карточек и локальной загрузки медиа, только если
  `PRODUCT_CATALOG_SYNC_REQUIRED=true`; флаг включают вместе с ежедневным
  `wbozon-product-catalog.timer`, поэтому старый ручной запуск не считается
  бессрочной аварией;
...
- последний статус и свежесть построения единого справочника товаров;
- свежесть нормализации карточек и результат локального скачивания медиа;
...
Каждый запуск записывается в `healthcheck_runs`. Для личного операционного бота значимыми событиями считаются только новый набор проблем и восстановление; неизменная ошибка и обычные успешные проверки не создают повторный личный дайджест.
