...
Проверки заданий Ozon включаются после установки timers:

```dotenv
OZON_REQUIRED_TASKS=products,orders,supplies,communications,daily_sales,finances
```

Добавляйте `ads` только при настроенных реквизитах Ozon Performance. Допустимый
возраст каждого задания задаётся переменными `OZON_*_MAX_AGE_SECONDS` из
`.env.example`.
