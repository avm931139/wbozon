...
Для некоторых документов при запросе логического формата, например `xlsx` или
`pdf`, Wildberries возвращает внешний файл `zip`. Worker принимает такой контейнер
только когда внутри действительно есть файл запрошенного расширения, сохраняет
ZIP без распаковки, а в `wb_document_files.extension` оставляет исходный логический
формат. Поэтому пакет считается загруженным и не попадает в бесконечную повторную
очередь.
