...
Справочник складов обновляется независимо от истории поставок. Если endpoint
`GET /api/v1/warehouses` временно недоступен, сервис сохраняет ранее загруженный
справочник, записывает предупреждение и продолжает загрузку списка, деталей,
товаров и упаковок поставок. Ошибки самих методов поставок по-прежнему завершают
задачу с ошибкой.
...
Предварительная воронка продаж загружается отдельным процессом:

```powershell
.\.venv\Scripts\python.exe -m wb.sales_funnel_sync
```

`wbozon-wb-sales-funnel.timer` запускает его каждый час. Endpoint
`/api/analytics/v3/sales-funnel/products/history` возвращает обновляемые WB раз в
час просмотры, корзины, заказы и выкупы по карточкам максимум за последние семь
дней. Они сохраняются
в `wb_sales_funnel_daily`; запуски — в `wb_sales_funnel_sync_runs`. Ошибка этого
процесса не откатывает и не останавливает общий WB worker, остатки или Order Feed.
В дашборде эта когортная аналитика показывается отдельным блоком и не подменяет
оперативную ленту заказов; неполный период явно помечается.
В том же цикле `/api/analytics/v3/sales-funnel/grouped/history` без фильтров
сохраняет кабинетные итоги в `wb_sales_funnel_account_daily`. Дашборд использует
их как основной итог воронки и показывает разницу с суммой по отдельным артикулам.
Это выявляет пропущенную карточку или разное время обновления двух ответов API.
Daily history не содержит отдельного счётчика отмен, поэтому код не считает
ожидающие заказы отменёнными: до закрытия финансового периода отмены берутся из
оперативной ленты.
