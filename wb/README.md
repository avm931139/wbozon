...
Последовательно запускает общий цикл WB без остатков и повторяет его через настроенный интервал. Остатки запускаются отдельно командой `python -m inventory_sync`. Одновременно может выполняться только один общий WB-цикл.
...
Запустить один полный цикл для проверки:
...
- Периодическая загрузка остатков вынесена в `inventory_sync`: исчезнувшие позиции FBS/FBO обнуляются, а в 01:00 по Москве создаётся отдельный исторический срез.
