...
На текущем этапе реализованы остатки, `GET /v2/campaigns`, справочники складов,
каталог через `offer-mappings` и ассортимент кампаний, а также актуальный
бизнес-метод заказов `POST /v1/businesses/{businessId}/orders`. Они разделены на
независимые tasks `identity`, `catalog`, `orders` и отдельный inventory-worker.

Практичный порядок следующих модулей:

1. **Статистика заказов и продаж:** `POST .../stats/orders` для агрегированных
   показателей и сверки оперативных заказов. Два устаревших `GET .../orders` не
   брать за основу.
2. **Цены:** бизнес-метод `POST /v2/businesses/{businessId}/offer-prices` плюс
   ценовой карантин. Не использовать устаревший `GET .../offer-prices`.
3. **Финансовые и аналитические отчеты:** платежи, услуги, реализация, ключевые
   показатели. Потребуется очередь асинхронной генерации, опрос `reportId` и
   загрузка готового файла.
4. **Акции и продвижение:** сначала read-only `promos`, `promos/offers`,
   `bids/info`; методы изменения включать только после отдельной защиты от
   случайной записи.
5. **Обратная связь:** отзывы, вопросы и чаты — отдельный модуль с доступом
