...
первого получения отчёта. Для частично заполненного дня запрашиваются только
отсутствующие виды отчётов, что экономит лимит генерации. Для API-ключа нужен доступ
...
Финансы и реклама изолированы по `business_id`: курсор истории, проверка
покрытия дат, замена диапазона и уникальные ключи включают идентификатор
кабинета. Поэтому синхронизация одного бизнеса не удаляет и не блокирует строки
другого. Если API неожиданно вернул строку с иным `businessId`, задача
останавливается до удаления ранее сохранённого диапазона. При наличии нескольких
бизнесов `YANDEX_MARKET_BUSINESS_ID` задаётся явно для каждого запуска.

`analytics` каждые шесть часов формирует официальный JSON-отчёт «Аналитика продаж»
