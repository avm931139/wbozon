...
Пакет `yandex_market` получает складские остатки из Partner API. Он подключён к общему `inventory_sync`, поэтому текущие строки обновляются каждый час, а в `00:00 Europe/Moscow` создаётся дневной срез вместе с WB и Ozon.
...
Для нескольких магазинов перечислите campaign ID через запятую. Токену достаточно read-only-доступа `offers-and-cards-management:read-only` или `all-methods:read-only`. Пока ключ или список кампаний не заполнены, этап Яндекс Маркета отключён и существующая синхронизация продолжает работать.
