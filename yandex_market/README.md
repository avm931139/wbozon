...
не более 30 календарных дней. Переход на повторную проверку последних
`YANDEX_MARKET_ORDER_LOOKBACK_DAYS` происходит только после записи успешного
полного запуска в `yandex_market_sync_runs`. Если первая историческая загрузка
...
московскую дату, а также статус последнего задания `orders`. Блок остаётся в
