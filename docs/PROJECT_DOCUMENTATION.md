...
- [`product_catalog/`](../product_catalog) — нормализация характеристик, история карточек и локальное хранилище фото/видео;
...
Яндекс Маркет API → yandex_market tasks / inventory@yandex_market ┤
...
Задание `finances` хранит дневной журнал `/v1/finance/accrual/by-day`, справочник
`/v1/finance/accrual/types` и детализацию до отправления/SKU через
`/v1/finance/accrual/postings`. Таблицы соответственно:
`ozon_finance_accruals`, `ozon_finance_accrual_types` и
`ozon_finance_posting_accruals`. Историческая детализация дозаполняется
ограниченными пакетами, чтобы не исчерпать лимит Seller API.
...
Для WB дополнительно сохраняется полный асинхронный отчёт `/api/v1/warehouse_remains` в `wb_warehouse_remains` и его дневная история в `wb_warehouse_remain_snapshots`. Дашборд и Telegram используют показатель `Всего находится на складах` из этого отчёта и детализацию по складам WB. FBS на собственном складе продавца в остаток WB не включается.
...
`product_catalog` после построения связей нормализует изображения, видео и характеристики всех кабинетов, создаёт новую историческую версию только при изменении карточки и докачивает отсутствующие файлы в `data/product_media`. Локальный файл проверяется по размеру и SHA-256. Ошибка одной ссылки не откатывает метаданные и другие файлы; она сохраняется для повторной попытки.

## Telegram-отчёты

- утренний отчёт содержит прошлый день и текущий месяц по вчера включительно;
- оперативный отчёт отправляется каждый час и показывает текущий день отдельными блоками Wildberries, Ozon и Яндекс Маркета;
- количество, сумма и схема заказов WB берутся из realtime WB Order Feed, который отдельный timer обновляет каждые 10 минут; Statistics API остаётся источником выкупов и возвратов;
- блок Ozon строится из `ozon_postings`, блок Яндекс Маркета — из `yandex_market_orders`; каждый показывает результат последней синхронизации заказов и не исчезает при нулевых данных или ошибке;
- дополнительно выводятся реклама и остатки WB и состояние WB-синхронизации;
...
- WB-файл содержит физический итог на складах WB, разбивку по складам и товары в пути без FBS продавца; Ozon-файл — агрегат и детализацию по складам с их названиями и кластерами;
