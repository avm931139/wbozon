...
- оперативный отчёт отправляется каждый час и показывает текущий день;
- количество, сумма и схема заказов берутся из realtime WB Order Feed, который отдельный timer обновляет каждые 10 минут; Statistics API остаётся источником выкупов и возвратов;
