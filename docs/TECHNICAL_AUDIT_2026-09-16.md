...
#### Статус исправления P1-03 на 17.09.2026

Основные опасные полные сканы устранены:

- WB finance, operational orders/sales и customer communications выбирают существующие строки только по ключам текущей порции; запросы автоматически разбиваются на ограниченные пачки;
- сводка качества ответов WB считается SQL-агрегатами без загрузки всей истории в Python;
- current stocks сначала обнуляются одним SQL `UPDATE`, после чего ORM загружает только ключи актуального ответа API;
- дневные снимки остатков создаются серверной операцией `INSERT ... SELECT`, без создания ORM-объекта на каждую строку;
- media и attributes каталога деактивируются массово и повторно загружаются только по ключам текущего каталога; проверка локальных файлов выполняется потоковой выборкой `yield_per(500)`.

Полный проход по исходным таблицам товаров и небольшим справочникам складов в product catalog/inventory оставлен намеренно: эти задания формируют полное текущее состояние, а не инкремент истории. Нагрузочный baseline 10x и унифицированные метрики RSS/inserted/updated/skipped остаются отдельной частью P1-03; поэтому пункт закрыт по риску полных сканов растущей истории, но не закрыт полностью по наблюдаемости и нагрузочному тестированию.
