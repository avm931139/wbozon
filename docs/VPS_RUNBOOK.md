...
После входа доступны четыре независимых раздела:

- `/` — оперативные заказы, выкупы, отмены, реклама и остатки;
- `/pnl` — точные закрытые финансовые данны WB, Ozon и Яндекс
  Маркета. Если отчёт не покрывает весь период, карточка остаётся
  недоступной и не подменяется оперативными заказами;
- `/stocks` — остатки по товарам на выбранную дату: локальные главные фото WB,
  Ozon и Яндекс, количество и время обновления записи в PostgreSQL.
- `/abc` — ABC-матрица SKU за финансовый период: выручка, прибыль, реклама и
  логистика по каждой площадке и в целом. Страница читает только
  `fact_product_economics_daily` и контрольный слой, а не исходные API-таблицы.
  Детализация рекламы отдельно хранится в `fact_advertising_daily`; поле
  `allocation_method` отличает прямую товарную статистику от распределения.

Для первой установки нужны `nginx`, `apache2-utils`, TLS-сертификат и пароль
HTTP Basic. Установщик не меняет `server.conf`, порт `1194/udp` или маршруты
OpenVPN. Он сначала проверяет существующий адрес `10.8.0.1/24`, а drop-in лишь
задаёт порядок запуска Nginx после VPN:
